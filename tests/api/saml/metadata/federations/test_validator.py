...
import tests.api.saml.fixtures
    ...
    @parameterized.expand(
        [
            (
                "incorrect_xml",
                utc_now(),
               tests.api.saml.fixtures.INCORRECT_XML,
                SAMLFederatedMetadataValidationError,
            ),
            (
                "without_valid_until_attribute",
                utc_now(),
               tests.api.saml.fixtures.FEDERATED_METADATA_WITHOUT_VALID_UNTIL_ATTRIBUTE,
                SAMLFederatedMetadataValidationError,
            ),
            (
                "with_expired_valid_until_attribute",
               tests.api.saml.fixtures.FEDERATED_METADATA_VALID_UNTIL
                + SAMLFederatedMetadataExpirationValidator.MAX_CLOCK_SKEW
                + datetime.timedelta(minutes=1),
               tests.api.saml.fixtures.FEDERATED_METADATA_WITH_VALID_UNTIL_ATTRIBUTE,
                SAMLFederatedMetadataValidationError,
            ),
            (
...
class TestSAMLMetadataSignatureValidator(object):
    @parameterized.expand(
        [
            (
                "without_signature",
               tests.api.saml.fixtures.FEDERATED_METADATA_CERTIFICATE,
               tests.api.saml.fixtures.FEDERATED_METADATA_WITH_VALID_UNTIL_ATTRIBUTE,
                SAMLFederatedMetadataValidationError,
            ),
            (
                "with_invalid_signature",
               tests.api.saml.fixtures.FEDERATED_METADATA_CERTIFICATE.strip(),
               tests.api.saml.fixtures.FEDERATED_METADATA_WITH_INVALID_SIGNATURE,
                SAMLFederatedMetadataValidationError,
            ),
            (
